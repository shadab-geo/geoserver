...
public class CacheFeatureSource extends DecoratingSimpleFeatureSource {
    ...
    public SimpleFeatureCollection getFeatures(Query srcQuery) throws IOException {
        ...
        if (candidateRequest == null && isSupported(strategy)) {
            LOGGER.log(Level.INFO, "Caching the request");
     ...
     * @throws IOException
     */
    private void writeFeatures(SimpleFeatureCollection features, MetadataMap metadata) throws IOException {
        long start = System.currentTimeMillis();
        try (SimpleFeatureIterator featureIterator = features.features(); FeatureWriter<SimpleFeatureType, SimpleFeature> writer = getFeatureWriter((String) metadata.get(SCHEMA_NAME), jdbcDataStore)) {
            int count = 0;
            while (featureIterator.hasNext()) {
                ...
                LOGGER.log(Level.INFO, "Time to cache: " + (end - start) + " ms");
            }
        }
    }
